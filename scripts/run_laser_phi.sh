...
expr_name=AReaL-GRPO-n4-streaming-phi
...
sink_win_size=128
recent_win_size=256
trial_name="laser_phi__${trial_name}"
...
python3 -m areal.launcher.local examples/lite/gsm8k_grpo.py --config examples/lite/configs/gsm8k_grpo.yaml \
    ...
    ++train_dataset.path=Kurt232/Sampled-Laser-phi-4-mini-reasoning \
    ++valid_dataset.path=Kurt232/Sampled-Laser-phi-4-mini-reasoning \
    ++actor.path="${model}" \
