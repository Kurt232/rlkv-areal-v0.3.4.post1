...
expr_name=AReaL-GRPO-n4-streaming-qwen14b
...
sink_win_size=128
recent_win_size=256
trial_name="qwen14b_cfg0_mcr16_t0.5__lr1e-2_ep2_bs32_reg2e-3__1"

python3 -m areal.launcher.local examples/math/gsm8k_grpo.py --config examples/rlkv/qwen14b_grpo.yaml \
    experiment_name="${expr_name}" \
    trial_name="${trial_name}" \
    total_train_epochs="${epochs}" \
    ++train_dataset.path=Kurt232/Sampled-Laser-Dataset-Qwen-2.5-14B-R1-v4.1 \
    ++valid_dataset.path=Kurt232/Sampled-Laser-Dataset-Qwen-2.5-14B-R1-v4.1 \
    ++actor.path="${model}" \
